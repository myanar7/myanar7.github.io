...
class JungleLogAppOptimized {
    ...
    async handleLogin(event) {
        ...
        try {
            ...
            console.log('🔍 Backend API ile login yapılıyor:', username);
            // Backend API ile login
            const loginResponse = await userServiceV2.login(username, password);
            console.log('👤 Login response:', loginResponse);
            
            if (!loginResponse.user) {
                console.log('❌ Login response\'da user bulunamadı');
    ...
    async updateRouteCompletion(routeId, isCompleted, attempts = null, score = 0) {
        try {
            ...
            if (isCompleted) {
                ...
                const saveResult = await routeCompletionService.saveRouteCompletion(completionData);
                
                // Backend'den gelen güncel stats bilgisini kullan
                if (saveResult && saveResult.userStats) {
                    // Stats verilerini güvenli hale getir
                    const safeStats = {
                        id: saveResult.userStats.id,
                    ...
                    appState.userStats = safeStats;
                    this.updateUserStatsDisplay();
                    console.log('✅ User stats backend\'den güncellendi:', safeStats);
                }
            ...
            // User stats artık backend'den geliyor, ayrıca çekmeye gerek yok

            // 2 saniye sonra cache invalidate + tekrar çek ve farklıysa güncelle
            setTimeout(async () => {
                try {
                    if (userStatsService.invalidateStatsCache) {
                        userStatsService.invalidateStatsCache(appState.currentUser.id);
                    }
                    ...
                    if (changed) {
                        ...
                        this.updateUserStatsDisplay();
                        console.log('✅ 2sn sonra user stats senkronize edildi:', latest);
                    }
                } catch (e) {
                    console.warn('⚠️ Gecikmeli user stats doğrulaması başarısız:', e);
                }
            }, 2000);
            
            // Route completions'ı yeniden çek
            console.log('🔄 Route completions yeniden çekiliyor...');
            try {
                await routeCompletionService.refreshUserCompletions(appState.currentUser.id, (snapshot) => {
                    appState.routeCompletions = snapshot.docs.map(doc => doc.data());
                    this.updateCheckboxStates();
                    console.log('✅ Route completions güncellendi:', appState.routeCompletions.length, 'completions');
                });
            } catch (completionsError) {
                console.error('❌ Route completions güncelleme hatası:', completionsError);
            }
            
            // HEMEN user stats çekme; backend'den gelen değer ekrana basıldı.
            // Gecikmeli doğrulama için cache invalidate + 2sn sonra kontrol yapılmalı (isteğe göre eklenebilir).
    ...
    // User stats display'ini güncelle
    updateUserStatsDisplay() {
        console.log('🔄 updateUserStatsDisplay çağrıldı');
    ...
    async autoLogin(username, password) {
        try {
            console.log('🔄 Otomatik giriş deneniyor:', username);
            
            // Backend login endpoint'ini kullan
            const loginResponse = await userServiceV2.login(username, password);
            ...
            if (loginResponse.user) {
                ...
                console.log('✅ Otomatik giriş başarılı:', user.username);
            }
        } catch (error) {
            console.error('❌ Otomatik giriş hatası:', error);
            // Cookie'yi temizle çünkü geçersiz
            this.deleteCookie('rememberedUser');
        }
    }
}
